...
async function generateMockAnalytics() {
  ...
  if (!site) {
    logger.error('No sites found. Please create a site first.');
    return;
  }

  logger.info(`Generating analytics for site: ${site.businessName}`);
  ...
  logger.info(`Generated analytics data for ${dates.length} days`);
}
  ...
  try {
    ...
    for (const dateRange of dateRanges) {
      ...
      if (site) {
        ...
        logger.info(`Found ${analytics.length} analytics records`);
        if (analytics.length > 0) {
          const totalViews = analytics.reduce((sum, a) => sum + a.views, 0);
          const totalLeads = analytics.reduce((sum, a) => sum + a.leads, 0);
          logger.info(`Total views: ${totalViews}, Total leads: ${totalLeads}`);
        }
      }
    }
    ...
    logger.info('GET /api/analytics/detailed?dateRange=30d');
    logger.info('GET /api/analytics/detailed?dateRange=7d&siteId={siteId}');
